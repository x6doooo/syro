...
UdpBase.defaultEventCallbacks = {
    ...
    'message': function (msg, rinfo) {
        ...
        // 记录来信方的最后一次时间
        self.friends[rinfo.address + ':' + rinfo.port] = {
            status: 'alive',
            lastUpdate: Date.now()
        };
...
proto.init = function(cfg) {
    ...
    */
    ...
    if (self.friendsTimer) {
        clearInterval(self.friendsTimer);
        self.friendsTimer = null;
    }
    
    self.friendsLiveTimeLimit = 15 * 1000;
    
    self.friendsTimer = setInterval(function() {
        var friends = self.friends;
        var line = Date.now() - self.friendsLiveTimeLimit;
        var tem;
    ...
    }, self.friendsLiveTimeLimit);

    /**
    * dgram控制器
    * @instance
    */
    var controller = self.controller = dgram.createSocket(cfg.sockeType || 
                                                          UdpBase.defaultConfig.sockeType);
